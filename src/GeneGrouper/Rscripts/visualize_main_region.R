...
for (ch in unique(df_chunk_order$chunk_group)){
  ...
  save_plot(paste(visualizations_dir,'/multiplot_repregions_', ch,'.png',sep='') ,p7,base_height=8,base_aspect_ratio = 2)
}
...
p1_taxafound <- ggplot(df_meta, aes(x=genus_dummy,y=value,fill=variable))+
  ...
  theme(legend.position='none',
        ...
        strip.text.x = element_blank())+
  ylab(label='Count')
save_plot(paste(visualizations_dir,'/taxa_found.png',sep=''),p1_taxafound,base_height=8,base_aspect_ratio = 2)
...
for (ch in unique(df_chunk_order$chunk_group)){
  ...
  save_plot(paste(visualizations_dir,'/region_taxa_breakdown_',ch,'.png',sep=''),p8,base_height=8,base_aspect_ratio = 2)
